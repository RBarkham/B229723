...
# Intro - Is there an assocation between antibiotic perscription rate and the seasons? How does this vary by healthboard?
...
# Our Packages
If you dont have any of these, copy the installation chunk below
```{R Package install, eval=FALSE, message=FALSE, warning=FALSE, class.source='fold-hide'}
# let's load our packages in 
...
If you'd like to access the data manually, you can create a CEDA account and download the data directly: https://catalogue.ceda.ac.uk/uuid/f02cc6ddd92f45b18b9ab6ab544df7d9/
...
#our data is in layers, one for each month. Currently the layers are named tasmin1, tasmin2 ect. This is difficult to understand. We use imaps() instead of maps here, becuase we want maps to keep a track of which year theyre from, and save it as .y. By using Names() we will rename these layers, (.x) applies it to both years; month.name is a in built vector which has all the months of the year. paste0() pastes whats insidie the brackets but removes any spaces the regular paste() function would have added - this is quicker than defining all the months manually. 
...
## Antibiotics Data
We can download our data directtly from the Website at : https://www.opendata.nhs.scot/dataset/prescriptions-in-the-community

```{R Reading in Antibioitics Data, eval=FALSE}
Antibiotics_data = c( 
...
```{r Reading in Antibiotic data (cont), message=FALSE, class.source='fold-hide'}
...
## you'll notice we had an error when we were loading "Parsing issue", in this instance we can ignore it, if you want to see more about it - look at the folded chunk below 
```
## Optional - Addressing Parsing Errors in Antibiotic Data
```{r Error investigation, class.source = 'fold-hide'}
map_df(names(Data_List_Antibiotics),
  ~ problems(Data_List_Antibiotics[[.x]]) %>%
      mutate(File = .x),  # tells us where thee error is
  .id = NULL
) ### we can see that there is missing data for Sucralfate (Nu), on Row Number 150,115 in Nov_2023 (our 6th tibble, and our 6th month)

error = Data_List_Antibiotics$`2023_November` %>% 
  slice(150114)
print(error) # we want to isolate the problematic row, so we will use slice() to isolate by row number. We have to take 1 off our number, to account for the fact the slice() doesn't count the header, like base r does in problems(} 
...
## Population Data
We'll be using data from Public Health Scotland again, to provide population statistics. If you'd like to manually download the data, it can be found here https://www.opendata.nhs.scot/dataset/population-estimates/resource/27a72cc8-d6d8-430c-8b4f-3109a9ceadb1
(Note, the website states popualtion dtaa is from 1981-2023, but actually streches to 2024 too)
...
pop = read_csv("https://www.opendata.nhs.scot/dataset/7f010430-6ce1-4813-b25c-f7f335bdc4dc/resource/27a72cc8-d6d8-430c-8b4f-3109a9ceadb1/download/hb2019_pop_est_03102025.csv") %>% 
  ...
  filter(year %in% c("2023", "2024") &
           sex == "All" &
           hb != "S92000003") %>% 
  dplyr::select(hb, all_ages, year) #base R also has a select(). R can get confued to which select() you want to use, so adding dplyr:: before select() helps R pick the correct one (the dplyr version)
```

## Map data
Now, we want ultimaltey want to plot our data as a chloropleths, so we need to give R a map (Like how you have a colouring book to provide a guide, so you knwo where to colour in).

We can download our map data from the Scottish Goverment website at: https://www.spatialdata.gov.scot/geonetwork/srv/eng/catalog.search#/metadata/f12c3826-4b4b-40e6-bf4f-77b9ed01dc14
...
## Temperature Data
```{r Raster --> DF conversion, message=FALSE, results='hide'}
...
ggplot(June_December_Antibiotics, aes(x = HBName, y = Dose_per_capita, fill = Month)) +
  ...
  facet_wrap(~ Year) +
  labs(x = " NHS Heath Board", y = "Antibiotic Doses per Capita",
       title = "Minimum Temperatures in June and December, by NHS Health Board") +
  theme_minimal() +
...
ggplot(June_December_Temp, aes(x = HBName, y = Min_Temperature, fill = Month)) +
  ...
  facet_wrap(~ Year) +
  theme_minimal() +
  labs(x = " NHS Heath Board", y = "Minimum Temperature",
       title = "Minimum Temperatures in June and December, by NHS Health Board") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
  plot.margin = margin(t = 10, r = 10, b = 10, l = 30))
...
ggplot(June_December_joined, aes(x = Min_Temperature, y = Dose_per_capita)) +
  ...
  labs(x = "Minimum Temperature", y = " Antibiotic Doses per Capita",
       title = "Scatter plot of Antibiotic Doses per Capita vs Minimum Temperature") +
  theme_minimal()
```
...
You can read up more Shiny.IO and Shiny here: 
https://shiny.posit.co/r/getstarted/shiny-basics/lesson1/
https://docs.posit.co/shinyapps.io/guide/

```{r Shiny App,class.source = 'fold-hide', eval=FALSE}
---
#Shiny web app, Whilst not shown, you will have to enable to your shiny.io connection with your unique token (see shiny.io user guide)
---
library(shiny)
library(scales)
library(sf)
...
From the plots we can see an inverse relationship between antibiotics prescription and minimum temperature - as it gets warmer, less antibiotics are prescribed.
...
This is not the only issues faced by seasonal perscriptions, as mentioned previously, over-perscription can promote antibiotic resistance. Future studies should also look at the bacterial:non-bacterial infection rate and compare it with the antibiotic doses per capita data. In regions where antibiotics are incorectly perscribed, we would be able to spot that baterial:non-bacterial infection rates are low with a disproportionatley high Antibiotic dosage per capita rate and could provide significant public benifits by helping curb antibiotic resistance development.
